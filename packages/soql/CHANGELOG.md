...
* ### Adds @soda3js/soql Package
  ...
  **Control flow** — `case_` with `{ when, result }` branch objects and a required `else` expression
  ...
  `SoQLDataType` enumerates all 21 column types returned by the SODA metadata endpoint. `SoQLTypeMap` maps each type to its JavaScript runtime shape for use in generic type parameters by higher-level packages. [#4][#4]

- ### Date/Time Functions

  Eight date\_extract\_\* functions for year, month, day, hour, minute, second, day-of-week, and week-of-year. Three date\_trunc\_\* functions for year, year-month, and year-month-day truncation.
  ...
  * `whereRaw(expression)` convenience method for raw WHERE clauses [#56][#56]
...
[#4]: https://github.com/soda3js/tools/pull/4
